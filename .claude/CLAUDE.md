...
- **Technical depth**: Assume infrastructure engineering expertise
...
- Always use markdown link format: [file.ts:42](path/to/file.ts#L42)
- Never use backticks or HTML for file references
...
- Never amend pushed commits or force push (including `--force-with-lease`)
- Create new commits to fix mistakes instead
...
- Always include ticket number at end: `[sc-XXXXXX]`
- If user hasn't provided a ticket number, ask for it before creating the PR
- Example: `"Add compliance agent role [sc-221186]"`

#### Structure

- **What**: Describe the changes
- **Why**: Explain the motivation
- **Testing**: How to verify the change worked *after* it's deployed - not the steps to deploy it
- **Release Notes**: User-facing changes (when applicable)

#### Labels

- Add `not release noted` label for infrastructure-only or internal PRs that are not customer-facing

### Repository Access & Tools

- Always check for gladly/sagansystems repos locally in `~/code/` first
...
- **Domain isolation**: Keep business logic pure and testable
- **Interface boundaries**: Use dependency inversion at layer boundaries
...
Local checkout location: `~/code/`

GitHub orgs: `gladly` (public-facing), `sagansystems` (internal/private)

### Key Repos

| Repo | Purpose | Tech |
|------|---------|------|
| `supernova` | Core platform backend | Golang, domain-driven design |
| `glados` | Platform frontend | TypeScript, Next.js |
| `thankful` | AI Standalone — connector to external helpdesks (Zendesk, Kustomer, Gorgias, Salesforce) | |
| `agent-desktop` | Gladly Team agent portal | |
| `gladly-claude-tools` | Claude Code configurations, agents, and skills | |
| `gladmin-cli` | CLI tooling | |

### Terminology

- **Gladly** = the AI customer support product
- **Gladly Team** = the human agent portal (formerly "Agent Desktop")
- Do not use "Sidekick" — deprecated branding
